...
/*
const pageKey = "scroll_" + location.pathname;

// 스크롤 위치 저장
window.addEventListener("beforeunload", () => {
  const target = document.querySelector(".target_list");
  if (target) {
    const offset = window.scrollY - target.offsetTop;
...
window.addEventListener("load", () => {
  ...
  if (target && saved !== null) {
    window.scrollTo(0, target.offsetTop + parseInt(saved));
  }
});*/
